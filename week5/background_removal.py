...
#Hough lines
def BackgroundMask420(img,img_path):
    large_number = 10000 # UNIMPORTANT PARAMETER, DO NOT CHANGE
    pict_start_perc = 0.4
    degrees_margin = 5
    initial_votes_perc = 0.2
    final_votes_perc = 0.05
    ...
    for votes_perc in np.arange(initial_votes_perc,final_votes_perc,-0.05):
        horizontal_lines = []
        vertical_lines = []
        votes_th = int(max(gray.shape)*votes_perc)
        lines = cv2.HoughLines(edges,1,np.pi/180,votes_th)
        if lines is None:
            continue
        for i in range(len(lines)):
            rho,theta = lines[i][0]
        ...
        if horizontal_lines and vertical_lines:
            ...
            vertical_lines = sorted(vertical_lines, key=lambda x: x["mean_point"])
            horizontal_lines = [item for item in horizontal_lines if min_border_distance < item["mean_point"] < img.shape[0]-min_border_distance]
            vertical_lines = [item for item in vertical_lines if min_border_distance < item["mean_point"] < img.shape[1]-min_border_distance]
            if len(horizontal_lines) >= 2 and len(vertical_lines) >= 2 and \
                    horizontal_lines[0]["mean_point"] < pict_start_perc*img.shape[0] and \
                    (1-pict_start_perc)*img.shape[0] < horizontal_lines[-1]["mean_point"] and \
                    vertical_lines[0]["mean_point"] < pict_start_perc*img.shape[1] and \
                    (1-pict_start_perc)*img.shape[1] < vertical_lines[-1]["mean_point"]:
                break                

        # cv2.imwrite(img_path.replace(".jpg","_lines.png"),img)
        # input('...')

    lines_wanted = {"top":horizontal_lines[0]["line"],
...
if __name__ == '__main__':
    ...
    img_paths = sorted(glob(os.path.join(imgs_folder, "*.jpg")))[:]
