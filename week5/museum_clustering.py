...
def main(descriptor='texture'):
    ...
    if descriptor == 'texture':
    ...
    elif descriptor == 'color_nosubblock':
        bbdd_descriptor = SubBlockDescriptor(bbdd_images, None)
        bbdd_results = bbdd_descriptor.compute_descriptors(grid_blocks=[1, 1])
        ...
        color_results = color_descriptor.compute_descriptors(grid_blocks=[1, 1])
        bbdd_results = {}
    ...
    cluster_folder = cluster_root + "_" + descriptor
    print("Done. Time: " + str(time.time() - start))
    ...
    for i in range(NUM_CLUSTERS):
        ...
        if cluster_size > 0:
            ...
            for row_num, data_idx in enumerate(data_idx_within_i_cluster):
                ...
                distances[data_idx] = distance.euclidean(one_row,
                                                         center_vec[0])  # np.linalg.norm(one_row - center_vec[0])
